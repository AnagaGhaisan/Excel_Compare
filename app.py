...
def upload_file():
    ...
    if k3_file and allowed_file(k3_file.filename, app.config['ALLOWED_EXTENSIONS']) and coretax_file_1 and allowed_file(coretax_file_1.filename, app.config['ALLOWED_EXTENSIONS']) and coretax_file_2 and allowed_file(coretax_file_2.filename, app.config['ALLOWED_EXTENSIONS']):
        ...
        try:
            k3_sheets = pd.read_excel(k3_file_path, sheet_name=None, header=1)  # Ensure header is read from row 2
            coretax_sheets_1 = pd.read_excel(coretax_file_path_1, sheet_name=None, header=1)  # Read with header in row 2
            coretax_sheets_2 = pd.read_excel(coretax_file_path_2, sheet_name=None, header=1)  # Read with header in row 2
        ...
        full_path, file_name, sheet_list = compare_files(k3_sheets, coretax_sheets_1, coretax_sheets_2, output_dir)
...
def show_comparison():
    ...
    if not os.path.exists(file_path):
        return f"File tidak ditemukan di: {file_path}. Pastikan mode={mode} sudah benar.", 404
    ...
    if mode == 'recap':
        ...
        for name in xls.sheet_names:
            ...
            html_content = df.to_html(classes='table table-hover table-bordered', index=False, na_rep='')
    ...
    # --- LOGIKA MODE COMPARE (Pagination & Pilih Sheet) ---
        ...
        table_html = page_data.to_html(classes='table table-hover table-striped table-bordered', index=False, na_rep='')
        ...
        for p in range(max(1, page - 2), min(total_pages, page + 2) + 1):
            if p == page:
                pagination_html += f' <span class="btn btn-sm btn-primary me-1">{p}</span>'
            else:
